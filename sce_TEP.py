...
def pre_model(model, traindata, train_attributelabel, testdata, testlabel, attribute_matrix):
    ...
    for i in range(test_pre_attribute.shape[0]):
        ...
        #print(pre_res)
        loc = (np.sum(np.square(attribute_matrix[i, :] - pre_res), axis=1)).argmin()
        #print(loc)
...
class Embedding_Net(nn.Module):
    ...
    def __init__(self,dim,lambda_):
        ...
        self.l11=nn.Linear(dim[0],dim[0])
        self.l12=nn.Linear(dim[0],dim[1])
        self.l13=nn.Linear(2*dim[1],dim[0])

        self.l21=nn.Linear(20,dim[0])
        self.l22=nn.Linear(dim[0],dim[1])
        self.l23=nn.Linear(2*dim[1],20)
...
#%%
modes=['NB'] #'rf'
test_classes={'test_class':[1,6,14]}
for key,value in test_classes.items():
    ...
    _,y_pre,y_true=pre_model(modes[0], traindata, train_attributelabel, testdata, testlabel, test_attributelabel)
    ...
    for epoch in range(200):
        ...
        with torch.no_grad():
            ...
            test_preattribute,label_lis, testlabel=pre_model(modes[0], f_train, train_attributelabel, f_test, testlabel, test_attributelabel)
            label_lis = np.asarray(label_lis)
